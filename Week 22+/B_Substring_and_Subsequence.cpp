...
void TEST_CASE(){
    ...
    for (int i = 0; i < b.length(); i++) {
        int longest_substring_till_now = 0;
        int starting_index = i;
        for (int j = 0; j < a.length(); j++) {
            if (a[j] == b[starting_index]) {
                starting_index++, longest_substring_till_now++;
            }
        }

        MAX = max(MAX, longest_substring_till_now);
    }
    
    cout << a.length() + b.length() - MAX << el;
