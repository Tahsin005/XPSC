...
// @Problem Link: 
bool is_parallel_to_X(ll x1,ll y1,ll x2,ll y2){
...
void solve(){
    ...
    if (is_parallel_to_X(x1, y1, x2, y2) and y3 < y1) {
        ans += abs(x1 - x2);
    }

    if (is_parallel_to_X(x1, y1, x3, y3) and y2 < y1) {
        ans += (abs(x1 - x3));
    }

    if (is_parallel_to_X(x2, y2, x3, y3) and y1 < y2) {
        ans += abs(x2 - x3);
    }
