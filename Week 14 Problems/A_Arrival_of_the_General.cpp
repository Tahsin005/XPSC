...
void TEST_CASE(){
    int n; cin >> n;
    vii v(n); for (int i = 0; i < n; i++) cin >> v[i];
    
    int nax = *max_element(all(v));
    int nin = *min_element(all(v));
    // cout << nin << " " << nax << el;
    int nax_idx = 0, nin_idx = 0;
    for (int i = 0; i < n; i++) {
        if (v[i] == nin) nin_idx = i;
    }
    for (int i = 0; i < n; i++) {
        if (v[i] == nax) {
            nax_idx = i;
    ...
    // cout << nin_idx << " " << nax_idx << el;
    if (nax_idx < nin_idx) {
        n--;
        cout << abs(nin_idx - n) + nax_idx << el;
        return;
    }
    n--;
    cout << abs(nin_idx - n) + nax_idx - 1<< el;
}
e4{
    #ifndef OJ
