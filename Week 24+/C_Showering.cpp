...
void TEST_CASE(){
    ...
    ll total_work_times = 0;
    vector<pair<ll, ll>> work_times;
    for (ll i = 0; i < n; i++) {
        ll x, y; cin >> x >> y;
        work_times.pb({x, y});
    }

    if (work_times[0].first - s >= 0) {
        cyes;
        return;
    } 
    if (work_times[n - 1].second + s <= m) {
        cyes; return;
    }
    ll max_interval = 0;
    for (ll i = 0; i < n - 1; i++) {
        ll window = abs(work_times[i].second - work_times[i + 1].first);
        max_interval = max(max_interval, window);
    }
    if (max_interval >= s) cyes;
    else cno;    
}
e4{
    #ifndef OJ
