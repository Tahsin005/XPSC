...
void TEST_CASE(){
    ...
    for (int i = 0; i < n; i++) {
        ...
        int lc = accumulate(baam.begin(), baam.end(), 0);
        int rc = accumulate(daan.begin(), daan.end(), 0);
        if ((int)(ceil(lc * 1.0 / 2 * 1.0)) <= baam[0] and (int)(ceil(rc * 1.0 / 2 * 1.0)) <= daan[1]) {
            int diff = abs(rc - lc);
            if (diff < val) val = diff, ans = i + 1;
        }
    }
    cout << ans << el;
}
