...
void TEST_CASE() {
    ...
    vector<pair<int64_t, int64_t>> lamps;
    for (int64_t i = 0; i < n; i++) {
        int64_t a, b;
        cin >> a >> b;
        lamps.push_back({a - 1, b});
    }

    sort(lamps.begin(), lamps.end(), [](pair<int64_t, int64_t> &p1, pair<int64_t, int64_t> &p2) {
    ...
    for (auto &lamp : lamps) {
        int64_t group = lamp.first;
        int64_t brightness = lamp.second;
