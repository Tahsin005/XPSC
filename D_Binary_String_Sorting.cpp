...
// @Problem Link:
#define c (ll)1000000000000
ll Last0(string s){
    int n = s.size();
    s = "#" + s;
    vll pref1(n + 2, 0), suff0(n + 2, 0);
    ...
    for (int i = 1; i <= n; i++){
        if (s[i] == '1' and i + 1 <= n and s[i + 1] == '0'){
            res = min(res, c + (c + 1LL) * (pref1[i] + suff0[i + 1] - 2LL));
        }
        else{
            res = min(res, (c + 1LL) * (pref1[i] + suff0[i + 1]));
        }
    }
...
ll First1(string s){
    ...
    for (int i = 1; i <= n; i++){
        if (s[i] == '0' and i - 1 >= 1 and s[i - 1] == '1'){
            res = min(res, c + (c + 1LL) * (pref1[i - 1] + suff0[i] - 2LL));
        }
        else{
            res = min(res, (c + 1LL) * (pref1[i - 1] + suff0[i]));
        }
    }
