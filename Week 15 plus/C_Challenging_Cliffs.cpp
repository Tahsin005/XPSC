...
void TEST_CASE(){
    ...
    if (n == 2) {
        cout << min(v[0], v[1]) << " " << max(v[0], v[1]) << el;
        return;
    }
    vii temp = v;
    sort(all(temp));
    ...
    for (int i = 0; i < n - 1; i++) {
        p[abs(temp[i] - temp[i + 1])] = make_pair(i, i + 1);
    }
    vector<pair<int,pair<int,int>>> vec(p.begin(), p.end());

    sort(all(vec));

    for (int i = vec[0].second.second; i < n; i++) {
        cout << temp[i] << " ";
    }
    for (int i = 0; i <= vec[0].second.first; i++) {
        cout << temp[i] << " ";
    }
    cout << '\n';
