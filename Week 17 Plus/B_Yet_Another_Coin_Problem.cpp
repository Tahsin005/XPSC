...
int minCoinsArray[101];

void calculateMinCoins() {
  minCoinsArray[0] = 0;

  for (int i = 1; i <= 100; ++i) {
    minCoinsArray[i] = imax;

    for (int j = 1; j <= 5 and j <= i; ++j) {
      int val = i - j * (j + 1) / 2;
      if (val >= 0 and minCoinsArray[val] < minCoinsArray[i]) {
        minCoinsArray[i] = minCoinsArray[val] + 1;
      }
    }
  }
}
